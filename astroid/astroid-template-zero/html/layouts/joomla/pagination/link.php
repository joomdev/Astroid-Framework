...
switch ((string) $item->text)
{
	...
	case Text::_('JLIB_HTML_START') :
		$icon = $app->getLanguage()->isRtl() ? 'fas fa-fast-forward fa-xs' : 'fas fa-fast-backward fa-xs';
	...
	case $item->text === Text::_('JPREV') :
		...
		$icon = $app->getLanguage()->isRtl() ? 'fas fa-forward fa-xs' : 'fas fa-backward fa-xs';
		$aria =Text::sprintf('JLIB_HTML_GOTO_POSITION', strtolower($item->text));
		break;

	// Check for "Next" item
	case Text::_('JNEXT') :
		$icon = $app->getLanguage()->isRtl() ? 'fas fa-backward fa-xs' : 'fas fa-forward fa-xs';
		$aria = Text::sprintf('JLIB_HTML_GOTO_POSITION', strtolower($item->text));
		break;

	// Check for "End" item
	case Text::_('JLIB_HTML_END') :
		$icon = $app->getLanguage()->isRtl() ? 'fas fa-fast-backward fa-xs' : 'fas fa-fast-forward fa-xs';
		$aria = Text::sprintf('JLIB_HTML_GOTO_POSITION', strtolower($item->text));
		break;
